...
task("deploy-liquidity-mining")
    ...
    const percPoolImplementation: TimeLockNonTransferablePool = await run("deploy-time-lock-non-transferable-pool-implementation", {
        verify: taskArgs.verify
    });
    ...
    const [percProxy, percProxyImplementation]: [TransparentUpgradeableProxy, TimeLockNonTransferablePool] = await run("deploy-proxy", {
        name: "Staked PERC",
        symbol: "sPERC",
    ...
    const percLPPoolImplementation: TimeLockNonTransferablePool = await run("deploy-time-lock-non-transferable-pool-implementation", {
        verify: taskArgs.verify
    });
    await percLPPoolImplementation.deployed();

    // Returns the same contract with two different interfaces: proxy and proxy implementation (interface of the pool)
    const [percLPProxy, percLPProxyImplementation]: [TransparentUpgradeableProxy, TimeLockNonTransferablePool] = await run("deploy-proxy", {
        name: "Staked PERC",
        symbol: "sPERC",
        depositToken: PERC, // users stake PERC tokens
        rewardToken: PERC, // rewards is PERC token
