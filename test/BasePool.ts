...
describe("BasePool", function () {
  ...
  describe("distributeRewards", async () => {
    ...
    it("Should fail when tokens are not approved", async () => {
      ...
      await expect(basePool.distributeRewards(DISTRIBUTION_AMOUNT)).to.be.revertedWith(
    ...
    it("Should work", async () => {
      ...
      expect(pointsPerShareAfter).to.eq(
        pointsPerShareBefore.add(DISTRIBUTION_AMOUNT.mul(pointsMultiplier).div(BASE_POOL_MINT_AMOUNT)),
      );
    });
  });
  describe("claimRewards", async () => {
    ...
    it("First claim single holder", async () => {
      ...
      expect(account2RewardTokenBalanceAfter).to.eq(
        account2RewardTokenBalanceBefore.add(DISTRIBUTION_AMOUNT1.sub(expectedEscrowed)),
      );
    ...
    it("Zero escrow", async () => {
      ...
      const tempBasePool = (
        await testBasePoolFactory.deploy(
          TOKEN_NAME,
          TOKEN_SYMBOL,
          depositToken.address,
          rewardToken.address,
