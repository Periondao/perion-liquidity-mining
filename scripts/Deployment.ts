...
async function deployUpgradeable() {
  ...
  percPoolProxy = new ethers.Contract(
    percPoolProxyDeploy.address,
    PERCPoolImplementationInterface,
    signers[0],
  ...
  percEthlpPoolProxy = new ethers.Contract(
    percEthlpPoolProxyDeploy.address,
  ...
  console.log(
    `  MULTISIG (${MULTISIG}) recieved DEFAULT_ADMIN_ROLE of percEthlpPoolProxy (${percEthlpPoolProxy.address})`,
    "\n\n",
  );
  ...
  console.log(
    `  -MULTISIG has DEFAULT_ADMIN_ROLE in both pools: ${adminRolePercPool}, ${adminRolePercEthlpPool}`,
    "\n",
  );
